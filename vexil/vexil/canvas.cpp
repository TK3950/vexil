...
Vexil::Canvas::Canvas(SDL_Renderer* r, SDL_Surface* s, SDL_Window* w, SDL_Event* e)
{
...
void Vexil::Canvas::UpdateToScreen()
{
	SDL_RenderCopy(renderer, texture, NULL, NULL);
	SDL_RenderPresent(renderer);
}

int Vexil::Canvas::GetAllEvents()
{
	while (SDL_PollEvent(events) != 0)
	{
		...
		if (events->key.keysym.scancode == SDL_SCANCODE_T && events->key.state == SDL_PRESSED)
		{
...
SDL_Renderer* Vexil::Canvas::getRenderer()
{
	return renderer;
}
